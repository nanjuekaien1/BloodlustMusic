...
if LOCALE == "enUS" then
	...
	L["Favored Friend filename: \n (NB: Needs filetype in the name, .ogg is most common. Also make sure the file for any custom song is in the 'customsongs' folder, not the 'sounds' folder)"] = "Favored Friend filename: \n |cFFFFD100(NB: Needs filetype in the name, .ogg is most common. Also make sure the file for any custom song is in the 'customsongs' folder, not the 'sounds' folder)|r"
	L["Favored Friend Name: "] = "Favored Friend Name: "
	L["Favored Friend Song Title: "] = "Favored Friend Song Title: "
	L["Mute"] = "|cFFFFD100Mute|r"
	L["Pending changes: \n \n \n'"] = "Pending changes: \n \n \n|cFFFFD100'"
	...
	L["BloodlustMusic:"] = "|cFFff2f00BloodlustMusic:|r "
...
if LOCALE == "zhCN" then
	...
	L["Cancel"] = "取消"
	L["Change Song filename to: \n (NB: Needs filetype in the name, .ogg is most common. Also make sure the file for any custom song is in the 'customsongs' folder, not the 'sounds' folder)"] = "将歌曲文件名改为：\n |cFFFFD100（注意：文件名中需要包含文件类型，.ogg 是最常见的。确保任何自定义歌曲的文件在 'customsongs' 文件夹中，而不是 'sounds' 文件夹）|r"
	...
	L["Favored Friend filename: \n (NB: Needs filetype in the name, .ogg is most common. Also make sure the file for any custom song is in the 'customsongs' folder, not the 'sounds' folder)"] = "挚友文件名：\n |cFFFFD100（注意：文件名中需要包含文件类型，.ogg 是最常见的。确保任何自定义歌曲的文件在 'customsongs' 文件夹中，而不是 'sounds' 文件夹）|r"
	L["Favored Friend Name: "] = "挚友姓名："
	L["Favored Friend Song Title: "] = "挚友歌曲标题："
	L["Mute"] = "|cFFFFD100静音|r"
	L["Pending changes: \n \n \n'"] = "待处理更改：\n \n \n|cFFFFD100'"
	...
	L["Sound not reset properly after Hero? Try setting your in-game volume at Main Menu -> System -> Sound to your preferred normal level, then click the button below."] = "英勇结束后声音未正确重置？尝试在主菜单 -> 系统 -> 声音中将游戏内音量设置为你喜欢的正常音量，然后点击下面的按钮。"
	L["Sound Reset"] = "声音重置"
	...
	L["This will set this song back to its default, hard-coded values. \n \n \n'"] = "这将把这首歌恢复到其默认的硬编码值。\n \n \n|cFFFFD100'"
	L["Use the Dropdown menu below to set what soundchannel you want Hero to play on. Unsure? Dialog is recommended, Master is more fun."] = "使用下面的下拉菜单设置你希望英勇在哪个声音通道播放。不确定？推荐使用主音道更有趣。"
	...
	L["Volume of "] = "音量为 "
	L["Want to customise what songs you hear? Below you can tick the boxes to turn individual songs on or off. You can press the edit button to change the names and change the song file itself. This way you can introduce your own custom songs without digging in the code.\n \nMessed up? Each edit button has a default button. Messed up a lot? At the bottom you'll see a button to put all songs back to their hard-coded values.\n\nFor more info on the whole process, please visit the addon site."] = "想要自定义你听到的歌曲吗？你可以在下面的勾选框来打开或关闭单独的歌曲。你也可以点击编辑按钮更改名称和歌曲文件本身。这样你可以引入自己的自定义歌曲，而无需深入修改代码。\n \n搞砸了？每个编辑按钮都有一个默认按钮。搞砸得很严重？在底部你会看到一个按钮，可以将所有歌曲恢复到默认。\n\n有关整个过程的更多信息，请访问插件网站。"
	L["Want to hear a certain song when a certain player casts Hero? You can by using the Favored Friend feature below. Just click the edit button and enter some values in the screen that pops up. You'll need the name of your friend, the title of the song, and the filename. The song can be either a song in the default list or a custom one, following the same rules for custom songs as above. More info on the addon page."] = "想在某个玩家施放英勇时听到特定的歌曲吗？你可以使用下面的挚友功能。只需点击编辑按钮并在弹出的屏幕中输入一些设置。需要输入你朋友的姓名、歌曲的标题和文件名。歌曲可以是默认列表中的歌曲或自定义歌曲，遵循上述自定义歌曲的相同规则。有关更多信息，请访问插件页面。"
	L["\n \nPending changes: \n \n'"] = "\n \n待处理更改：\n \n|cFFFFD100'"
	L["\n \nThis will set this song back to its default, hard-coded values. \n \n'"] = "\n \n这将把这首歌恢复到默认。\n \n|cFFFFD100'"
